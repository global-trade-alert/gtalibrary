...
gta_cumulative_counts=function(
    counts.by = 'quarter',coverage.period = NULL,
    data.path = "data/master_plus.Rdata",gta.evaluation= NULL,
    affected.flows = NULL,implementing.country = NULL,
    keep.implementer = TRUE,affected.country = NULL,
    keep.affected = NULL,incl.affected.strictness="ONEPLUS",
    keep.others=TRUE, nr.affected=c(0,999),
    nr.affected.incl="ALL",announcement.period = NULL,
    implementation.period = NULL,keep.implementation.na = NULL,
    revocation.period = NULL,keep.revocation.na = TRUE,
    submission.period = NULL,in.force.on.date = Sys.Date(),
    keep.in.force.on.date = 'any',intervention.types = NULL,
    keep.type = NULL,mast.chapters = NULL,
    keep.mast = NULL, implementation.level = NULL,
    keep.level = NULL,eligible.firms = NULL,
    keep.firms = NULL,cpc.sectors = NULL,
    keep.cpc = NULL,hs.codes = NULL,
    keep.hs = NULL,intervention.ids = NULL,
    keep.interventions = NULL,lag.adjustment=NULL,
    reporting.period=NULL,df.name="master.sliced",
    pc.name="parameter.choice.slicer",xlsx = FALSE,
    output.path = NULL
) {
  ...
  gta_data_slicer(
      data.path = data.path,
      gta.evaluation = gta.evaluation, affected.flows = affected.flows,
      implementing.country = implementing.country, keep.implementer = keep.implementer,
      affected.country = affected.country, keep.affected = keep.affected,
      incl.affected.strictness = incl.affected.strictness, keep.other = keep.others,
      nr.affected = nr.affected, nr.affected.incl = nr.affected.incl,
      announcement.period = announcement.period, implementation.period = implementation.period, keep.implementation.na = keep.implementation.na,
      revocation.period = revocation.period, keep.revocation.na = keep.revocation.na,
      submission.period = submission.period, in.force.on.date = in.force.on.date,
      ...
      keep.mast = keep.mast, implementation.level = implementation.level,
      keep.level = keep.level, eligible.firms = eligible.firms,
      keep.firms = keep.firms, cpc.sectors = cpc.sectors,
      keep.cpc = keep.cpc, hs.codes = hs.codes,
      keep.hs = keep.hs, intervention.ids = intervention.ids,
      keep.interventions = keep.interventions, lag.adjustment = lag.adjustment,
      reporting.period = reporting.period, df.name = df.name,
      pc.name = pc.name, xlsx = xlsx, output.path = output.path
    )
