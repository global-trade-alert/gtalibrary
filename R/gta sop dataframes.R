...
gta_sop_dataframes <- function(aff.jur.df = F, dist.market.df = F, int.level.df = F, hs.codes.df = F, cpc.codes.df = F,
    ...
    # master
    nam.amas <- data.frame(df = "master", nam = c(
        "intervention.id", "state.act.id", "title", "announcement.description", "source",
    ...
    # int.level.df
    if (int.level.df) {
        nam.eint.lev <- data.frame(df = "int.level.df", nam = c(
            "intervention.id", "prior.level", "new.level", "tariff.peak", "level.unit",
            "level.type.id"
        ))
    }
    ...
    # hs.codes.df
    if (hs.codes.df) {
        nam.fhs.cod <- data.frame(df = "hs.codes.df", nam = c(
            "intervention.id", "hs.codes", "inception.date", "removal.date", "prior.level",
    ...
    # firms.df
    if (firms.df) {
        nam.hfirms <- data.frame(df = "firms.df", nam = c(
            "firm.name", "firm.name.original", "role.id", "location", "assignment.id",
    ...
    sop.data.dump <- data.table::rbindlist(mget(ls(pattern = "nam.")), fill = T, use.names = T)
    sop.data.dump <- data.frame(sop.data.dump)
    sop.data.dump$df <- as.character(sop.data.dump$df)
    sop.data.dump$nam <- as.character(sop.data.dump$nam)
