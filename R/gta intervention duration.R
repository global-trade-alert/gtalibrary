...
gta_intervention_duration <- function(data = NULL, ## the function does not show zeros anymore --> needed ?
    ...
    # dplyr::filter(date.implemented < date.removed & dplyr::between(lubridate::year(date.implemented), years[1], years[2]) & dplyr::between(lubridate::year(date.removed), years[1], years[2]))
    ...
    results <- datefunction(
        ...
        current_year_todate = current.year.todate
    )
