...
gta_intervention_duration <- function(
  ...
  if (is.data.frame) {
    ...
    if (ncol(master) < 3) {
      stop("Please supply a data frame with at least three columns (intervention ID, implementation and removal date).")
    }
    if (ncol(master) == 3) {
  ...
  if (is.null(years)) {
    ...
    if (length(years) != 2) {
      stop("Please supply a year vector with two entries e.g. c(2008, 2018)")
    }
    if ((min(years) < 2008) | (max(years) > year(Sys.Date()))) {
      stop(paste("Please only supply years between 2008 and ", year(Sys.Date()), sep = ""))
    }
    if (is.numeric(years) == F) {
      stop("Please supply a year vector with two integer entries e.g. c(2008, 2018)")
    }
